...
def huber_loss(y_true, y_pred, clip_value):
    ...
    if K.backend() == 'tensorflow':
        ...
        print('[ERROR] theano is not supported at this poing.')
    else:
...
class Hdqn:
    def __init__(self, gpu):
        ...
        with tf.device('/{0}:'.format(device) + str(gpu)):  # change this line to GPU
            ...
            controllerTarget.add(Dense(HIDDEN_NODES, activation='relu',
                                       kernel_initializer=initializers.random_normal(stddev=0.01, seed=SEED)))
            controllerTarget.add(Dense(nb_Action, activation='linear',
                                       kernel_initializer=initializers.random_normal(stddev=0.01, seed=SEED)))
